...
extension TerminalView {
    ...
    /// LFG PATCH: output chunks at or below this size paint immediately.
    enum LFGPatch {
        static let immediateDisplayMaxBytes = 256
    }
    ...
    func feedFinish (byteCount: Int = Int.max)
    {
        suspendDisplayUpdates ()
        // LFG PATCH (see PATCHES.md): a small chunk on the main thread is almost
        // always a keystroke echo. Paint it in this run-loop turn instead of
        // after the 1/60s throttle, which cost ~10-20ms per typed character.
        // Larger chunks keep the throttle, so full-screen repaints that arrive
        // in several reads still coalesce and don't flicker.
        if byteCount <= LFGPatch.immediateDisplayMaxBytes && Thread.isMainThread {
            updateDisplay()
        } else {
            queuePendingDisplay()
        }
    ...
    public func feed (byteArray: ArraySlice<UInt8>)
    {
        ...
        feedFinish(byteCount: byteArray.count)
    }

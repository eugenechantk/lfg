...
enum Config {
    ...
    struct HostEntry: Decodable, Equatable {
        let url: String
        let ssh: String?
    ...
    struct HostsFile: Decodable { var hosts: [HostEntry] }
    struct SeedHostsFile: Encodable { var hosts: [String] }
    ...
    static func loadHosts() -> [HostEntry] {
        ...
        let seed = SeedHostsFile(hosts: ["http://localhost:8766"])
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        if let data = try? JSONEncoder().encode(seed) {
            try? data.write(to: hostsFile)
        }
        return seed.hosts.map { HostEntry(url: $0) }
    }

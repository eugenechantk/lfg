...
// Verbatim shape from ~/.claude/projects/-Users-eugenechan-dev-inbox/3402b3b2….jsonl
// (Claude Code 2.1.278): the caveat row is isMeta, the other two are not.
const localCaveat = () => ({
  type: "user",
  isMeta: true,
  message: {
...
const localStdout = (out: string) => ({
  type: "user",
  message: { role: "user", content: `<local-command-stdout>${out}</local-command-stdout>` },
});
const localStderr = (err: string) => ({
  type: "user",
  message: { role: "user", content: `<local-command-stderr>${err}</local-command-stderr>` },
});
...
describe("classifyTurnLine", () => {
  ...
  // A LOCAL slash command (/login, /model, /chrome) runs inside the TUI and
  // never reaches the model: Claude Code writes the command and its stdout as
  // ordinary (non-meta) user rows and nothing follows — no assistant row, no
  // turn_duration. Reading them as "a user record means mid-turn" pinned a
  // session that had only run /login as Working until STALL_MS demoted it
  // (cy-122914-44677, 2026-09-22). They are not decisive either way: a local
  // command can also run mid-turn, so `idle` here would close a live turn.
  test("local slash command rows are not decisive", () => {
    for (const r of [localCommand("/login"), localStdout("Login successful"), localStderr("nope")]) {
      expect(classifyTurnLine(JSON.stringify(r))).toBeNull();
    }
  });
});
...
describe("transcriptTurnState", () => {
  ...
  // The bug as observed: a fresh session whose only activity is /login. Nothing
  // decisive exists, so the transcript abstains and the pane (an empty
  // composer) decides idle.
  test("fresh session that only ran /login → null, not running", async () => {
    const p = write([
      { type: "last-prompt" },
      { type: "mode" },
      ...
      localCaveat(),
      localCommand("/login"),
      localStdout("Login successful"),
    ]);
    expect(await transcriptTurnState(p)).toBeNull();
  });

  test("local command after a finished turn → still idle", async () => {
    const p = write([prompt("go"), assistant("done"), turnEnd(), localCaveat(), localCommand("/model"), localStdout("Set model to opus")]);
    expect(await transcriptTurnState(p)).toBe("idle");
  });

  test("local command run mid-turn does not close the turn", async () => {
    const p = write([turnEnd(), prompt("go"), toolUse(), toolResult(), localCaveat(), localCommand("/model"), localStdout("Set model to opus")]);
    expect(await transcriptTurnState(p)).toBe("running");
  });

  test("a real prompt after a local command → running", async () => {
    const p = write([localCommand("/login"), localStdout("Login successful"), prompt("now do the thing")]);
    expect(await transcriptTurnState(p)).toBe("running");
  });

...
| SC2 | `desktop/build/lfg.app/Contents/MacOS/lfg --desktop-feature-test` | PASS — 127 assertions, including exact-directory ranking, trailing-slash normalization, newest readable model, agent-default fallback, and no-match behavior. |
...
| SC5 | `desktop/build.sh`; feature suite; `git diff --check` | PASS — build succeeds, 127 assertions pass, whitespace check is clean. Existing dirty-tree work was preserved. |
